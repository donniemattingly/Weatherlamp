...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(16, D6);
uint32_t color;
uint8_t brite;
int changeColor;
void setup() {
  ...
  Spark.function("powerOff", powerOff);
}

int setColor(String strColor){
    char rChar[4];
    char gChar[4];
    char bChar[4];
    
    strColor.substring(0,3).toCharArray(rChar,4);
    strColor.substring(3,6).toCharArray(gChar,4);
    strColor.substring(6,9).toCharArray(bChar,4);
    
    uint8_t r = atol(rChar);
    uint8_t g = atol(gChar);
    uint8_t b = atol(bChar);
